...
class AverageDownSample(gp.BatchFilter):
    ...
    def setup(self):
        ...
        assert self.target_voxel_size % self.source_voxel_size == gp.Coordinate((0,) * len(self.target_voxel_size)), (
            f"{self.target_voxel_size % self.source_voxel_size}"
        )
        assert self.target_voxel_size > self.source_voxel_size, f"{self.target_voxel_size=} {self.source_voxel_size=}"
