"""Fixed version of gunpowder's Pad node that handles requests entirely outside upstream ROI."""
...
class Pad(BatchFilter):
    ...
    This is a fixed version that properly handles cases where the requested ROI
    lies entirely outside the upstream ROI.
    ...
    def process(self, batch, request):
        ...
        if isinstance(self.key, ArrayKey):
            ...
            # Get the ROIs in voxel space
            from_roi = array.spec.roi / array.spec.voxel_size
            to_roi = request[self.key].roi / array.spec.voxel_size

            # Check if the upstream returned an empty array (request was entirely outside)
            if from_roi.empty or array.data.size == 0:
                # Create an array filled entirely with the pad value
                value = self.value if self.value is not None else 0

                # Determine the number of channels from the original array shape
                num_channels = len(array.data.shape) - from_roi.dims if array.data.size > 0 else 0

                if num_channels > 0:
                    shape = array.data.shape[:num_channels] + tuple(to_roi.shape)
                else:
                    shape = tuple(to_roi.shape)

                array.data = np.full(shape, value, dtype=array.data.dtype)
            else:
