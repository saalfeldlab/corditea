...
def test_basic_2d_lsd_computation(array_keys, simple_2d_segmentation):
    ...
    lsd_node = AddLSD(segmentation=seg_key, descriptor=desc_key, sigma=0.5)
...
def test_3d_lsd_computation(array_keys, simple_3d_segmentation):
    ...
    source = gp.ArraySource(seg_key, PersistenceArray(simple_3d_segmentation, voxel_size=voxel_size))

    lsd_node = AddLSD(segmentation=seg_key, descriptor=desc_key, sigma=1.5)
...
def test_with_labels_mask(array_keys, simple_2d_segmentation):
    ...
    source = (
        gp.ArraySource(seg_key, PersistenceArray(simple_2d_segmentation, voxel_size=voxel_size)),
        gp.ArraySource(labels_mask_key, PersistenceArray(labels_mask, voxel_size=voxel_size)),
    ) + gp.MergeProvider()

    lsd_node = AddLSD(
        segmentation=seg_key, descriptor=desc_key, lsds_mask=mask_key, labels_mask=labels_mask_key, sigma=1.0
    )
...
def test_downsample_parameter(array_keys, simple_2d_segmentation, downsample_factor):
    ...
    lsd_node = AddLSD(segmentation=seg_key, descriptor=desc_key, sigma=1.0, downsample=downsample_factor)
...
def test_roi_cropping(array_keys, simple_2d_segmentation):
    ...
    lsd_node = AddLSD(segmentation=seg_key, descriptor=desc_key, sigma=1.0)
...
def test_scale_invariance(array_keys):
    ...
    for config in configs:
        ...
        source = gp.ArraySource(seg_key, PersistenceArray(base_data, voxel_size=voxel_size))
    ...
    np.testing.assert_allclose(
        desc1, desc2, rtol=1e-1, atol=1e-2, err_msg="LSDs should be similar for equivalent physical configurations"
    )
...
def test_singleton_channel_dimension(array_keys, simple_2d_segmentation):
    """Test that AddLSD handles singleton channel dimensions correctly."""
    seg_key = array_keys["segmentation"]
    desc_key = array_keys["descriptor"]
    ...
    # This should not crash with dimension mismatch
    with gp.build(pipeline):
        batch = pipeline.request_batch(request)
